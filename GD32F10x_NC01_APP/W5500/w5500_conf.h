...
#define ApplicationAddress 	    0x08010000  // 0x08000000-0x08007FFF Boot size is 32K
#define AppBackupAddress        0x08030000  // 0x08008000-0x08017FFF App back start address and size is 64k
#define ConfigAddr		          0x0800f000  // 0x08018000-0x08027FFF APP backup and configaddr start address is 64k 
#define verAddr                 0x0800f800  // 版本信息存储   
...
typedef struct _CONFIG_MSG											
{
  ...
  uint8 state;
	...
	uint32 reset_D_latch_time;// reset D latch time
	
	uint32_t barcode_trigger_pressed_time;//barcode trigger time

	uint8_t reserved;
	uint8_t batch;//batch
	uint16_t sn;//
	uint16_t key;//	
	int16_t endmark;//update FW,should set to 0xAA
	//control floor and ceil
	int16_t tmp_ctrl_floor;//temperature control floor
	int16_t tmp_ctrl_ceil;//temperature control ceil
	int16_t humidity_ctrl_floor;//humidity control floor
	int16_t humidity_ctrl_ceil;//humidity control ceil
	//warn floor and ceil
	int16_t tmp_warn_floor;//temperature warn floor
	int16_t tmp_warn_ceil ;//temperature warn ceil
	int16_t humidity_warn_floor;//humidity warn floor
	int16_t humidity_warn_ceil;//humidity warn ceil
	//compensate
	int16_t tmp_compensation;//temperature compensation
	int16_t humidity_compensation;//humidity compensation
	//
	int16_t open_beep_delay_time;//entrance and beep delay time
